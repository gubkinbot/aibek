...
- Хранения кодов верификации email и кодов сброса пароля (TTL 10 мин)
- Realtime данные от OPC-коллектора (pub/sub каналы и snapshots)
- Статусы подключений к OPC-серверам (TTL 30 сек)
- Метрики Docker-контейнеров и сервера (Redis Streams)
- Настройки доступа по умолчанию для новых пользователей
...
## OPC UA (компрессорные станции)

::: info
Параметры OPC UA (URL серверов, сертификаты, интервалы) настраиваются **не через .env**, а через веб-интерфейс или API:
- `POST /api/modules/compressor/stations` — создание станции с OPC-параметрами
- `POST /api/modules/compressor/stations/{code}/generate-cert` — генерация сертификата

Сертификаты сохраняются в директории `opc-certs/` (монтируется read-only в контейнер коллектора).
:::
...
|--------|-----------|---------|----------|
| Nginx | 80 | **80** | Единая точка входа + WebSocket |
| Backend | 8000 | 8000 | API (через Nginx: `/api/`) |
| OPC Collector | — | — | Сбор данных с OPC UA серверов |
| Analytics | — | — | Детекция аномалий |
| Frontend | 5173 | 5173 | Интерфейс (через Nginx: `/`) |
| Docs | 5174 | 5174 | Документация (через Nginx: `/docs/`) |
| TimescaleDB | 5432 | 5432 | База данных |
| Redis | 6379 | 6379 | Кэш, pub/sub, realtime |

::: info
В production рекомендуется закрыть все порты кроме 80 (Nginx) — все сервисы доступны через Nginx. OPC Collector и Analytics не имеют внешних портов.
:::
